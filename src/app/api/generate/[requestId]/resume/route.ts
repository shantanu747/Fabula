...
import { recordStreamResume } from "@/lib/observability/metrics";
...
export const GET = withRoute(
  "/api/generate/[requestId]/resume",
  async (request: Request, { params }: RouteContext<"/api/generate/[requestId]/resume">) => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        // Reached by every path out of the loop above that isn't an early
        // `return`: the client disconnected (clientGone), the buffer expired
        // mid-poll, or MAX_WAIT_MS elapsed with the generation still running —
        // none of those delivered a terminal frame to this caller.
        if (!clientGone) recordStreamResume("timed_out");
        closeIfPresent();
      },
      cancel() {
        clientGone = true;
