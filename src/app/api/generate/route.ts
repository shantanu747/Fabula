...
import { getActiveRequestId, readActiveRoundtripCount } from "@/lib/observability/requestContext";
import {
  generationEnded,
  generationStarted,
  recordAdmissionRejected,
  recordBudgetExceeded,
  recordCachePromptFromUsage,
  recordDbRoundtrips,
  recordGenerationCostUsd,
  recordGenerationDuration,
  recordGenerationOutcome,
  recordGenerationTokens,
  recordGenerationTtft,
...
/**
 * The one high-volume, non-error log event this route emits once per
 * generation regardless of outcome — a candidate for sampling under real
 * load (docs/adr/0049). Defaults to full (1); configurable via env rather
 * than hardcoded, same "parse or fall back sanely" pattern as
 * `policy.ts`'s `trustedProxyHopCount`. `GENERATE_STARTED`/`_COMPLETED`/
 * `_FAILED`/`_CANCELLED` are not sampled — only this one, and only via
 * `log.info`'s own `sampleRate` option, which does not exist on `log.warn`/
 * `log.error` at all (errors are never sampled, structurally).
 */
function firstChunkLogSampleRate(): number {
  const raw = process.env.LOG_SAMPLE_RATE_GENERATE_FIRST_CHUNK;
...
export const POST = withRoute(
  ...
  async (request: Request) => {
    // `withRoute` already resolved and validated this request's id — reusing
    // it (rather than calling `resolveRequestId` again here) is what keeps a
    // single request id on the response header, the `http.route` span, and
    // every log line this handler emits. Falls back to resolving it directly
    // only when this handler runs outside `withRoute`'s context (a unit test
    // calling `POST` before this module's own OTel context manager is set up
    // — see requestContext.ts's own doc comment).
    const requestId = getActiveRequestId() ?? resolveRequestId(request);
    ...
    if (!lease.acquired) {
      ...
      recordAdmissionRejected(lease.reason);
      return withRequestId(
        Response.json(
          {
    ...
    if (hasDatabase()) {
      ...
      if (!budget.allowed) {
        ...
        // budget.kind is only ever absent when allowed is true (budget/index.ts's
        // own BudgetCheckResult doc comment) — guaranteed present on this branch.
        recordBudgetExceeded(budget.kind as NonNullable<typeof budget.kind>);
        return withRequestId(
          Response.json(
    ...
    async function finish(args: FinishArgs) {
      ...
      recordGenerationDuration(args.totalMs, { provider: input.providerId, outcome: args.outcome });
      recordGenerationOutcome({ provider: input.providerId, outcome: args.outcome });
      if (args.ttftMs !== undefined) {
        recordGenerationTtft(args.ttftMs, {
      ...
      if (args.result?.usage) {
        ...
        recordGenerationTokens(usage.inputTokens, { provider: input.providerId, kind: "input" });
        recordGenerationTokens(usage.outputTokens, { provider: input.providerId, kind: "output" });
        if (usage.cacheReadInputTokens !== undefined) {
          recordGenerationTokens(usage.cacheReadInputTokens, { provider: input.providerId, kind: "cache_read" });
        }
        if (usage.cacheCreationInputTokens !== undefined) {
          recordGenerationTokens(usage.cacheCreationInputTokens, { provider: input.providerId, kind: "cache_write" });
        }
        recordCachePromptFromUsage(input.providerId, usage);
      }
      if (costUsd !== undefined) {
        recordGenerationCostUsd(costUsd, { provider: input.providerId, model: args.result?.model ?? "unknown" });
      }
      ...
      // withRoute's own automatic recording is skipped for this route
      // (selfReportsRoundtrips: true below) — its handler promise resolves
      // long before the stream above finishes making its own database calls,
      // so it would under-report. finish() is this route's one true
      // completion point regardless of outcome, reached after every database
      // call this request is ever going to make — including the
      // insertGenerationEvent write just above — so the context-propagated
      // counter (requestContext.ts) is complete by the time it's read here.
      recordDbRoundtrips(readActiveRoundtripCount(), "/api/generate");
    }
    ...
    log.info(
      LOG_EVENTS.GENERATE_FIRST_CHUNK,
      { requestId, providerId: input.providerId, ttftMs },
      { sampleRate: firstChunkLogSampleRate() }
    );
