...
**Typecheck.** `npm run typecheck` is `next typegen && tsc --noEmit`, not bare `tsc --noEmit`.
Next 16's ambient `RouteContext`/`PageProps`/`LayoutProps` types (used for typed dynamic-route
params) live in `.next/types/`, generated by `next dev`/`next build` — on a cold checkout with no
prior `.next`, which is exactly what CI's `quality` job is, plain `tsc --noEmit` fails with
`Cannot find name 'RouteContext'` and similar. `next typegen` generates just those types, without
a full build, in well under a second — this was caught by the `quality` job's own first real CI
run (a fresh checkout), not locally, where `.next/types` already existed from prior builds.
`tsconfig.json` already includes `**/*.ts`/`**/*.mts` across `evals/`, `test-support/`, `e2e/`, and
`scripts/` — intentionally, so this one command covers all of it.
