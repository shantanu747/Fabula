...
6. **Back in `streamGeneration`**, once the stream ends, any parsed metadata is passed to `onDone(finalText, metadata)`. A stream that breaks for transport reasons (not a clean `error`/`done` frame) attempts one resume against `GET /api/generate/[requestId]/resume` before falling back to the pre-existing generic error.
