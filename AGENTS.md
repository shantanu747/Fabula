...
- No feature add, bug fix, hotfix, or other edit to this repo is complete until the conditions that make the `build` GitHub Action pass are actually true of the working tree — not just "it would probably be fine." This applies to every change, not just large ones. This does not mean committing and opening a PR to watch Actions run; it means locally reproducing what that workflow does closely enough that its outcome is already known before it ever runs.
  - Run every step `.github/workflows/ci.yml` runs. The `build`, `quality`, and `e2e` jobs run in parallel, not chained, so "in the same order" means within each job: `npm run lint`, `npm test` (the `db` project needs a real Postgres reachable at `TEST_DATABASE_URL`, default `postgres://postgres:postgres@localhost:5432`; `docker run -d -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -p 5432:5432 postgres:17-alpine` matches what CI provisions), `npx drizzle-kit check`, `npm run test:coverage`, `npm run eval` (replay-only, no keys — fails on stale prompt fixtures; see ADR 0018), and `npm run build` (the `build` job); `npm run typecheck`, `npm run test:scripts`, `npm run build`, and `npm run bundle-budget` (the `quality` job — needs a fresh `.next`: `rm -rf .next` before trusting a number, since a cached build changes chunk hashes and sizes; see ADR 0027); and `npm run test:e2e` (the `e2e` job — needs both the Postgres above and the Neon HTTP proxy from README's "Developing against a local database" section reachable at `db.localtest.me:4444`; see `e2e/global-setup.ts`, which fails loudly with setup instructions if either is missing, ADR 0019, and ADR 0027 for the responsive/accessibility specs this job also runs).
...
- **Diagnosing e2e flakiness (`e2e/specs/`): don't trust `[WebServer]`-prefixed log line order as evidence of causation.** Playwright's webServer output is a separate, asynchronously-piped stream from its own test reporter, and the two interleave in whatever order Node happens to flush them — not the order events actually happened in. A theory built on "this error appeared right before the failure in the CI log" is not verified; confirm it by actually reproducing the failure (see below), or by reading the failing test's own `trace.zip`/`error-context.md` artifacts, before writing a fix. See ADR 0020 for a case where this produced a plausible-looking but wrong fix that shipped, then failed again in CI with a different symptom.
  - Before declaring an e2e-flakiness fix verified, run `npm run test:e2e:soak` (repeats the full suite; ~10 min), not a single `npm run test:e2e` pass — a flake that failed twice in two CI attempts can pass 5/5 locally by construction (a full suite run restarts the webServer from scratch each time, so it never puts much traffic through one connection pool/process) without the fix addressing anything. To hunt down which spec interaction triggers a suspected race, target it directly: `npx playwright test --config e2e/playwright.config.ts --repeat-each=15 <spec-a> <spec-b>` runs those files' tests interleaved and repeated on one continuous server, which is a much smaller, faster loop than restarting the whole suite.
  - If a fix's local soak run comes back clean but the theory behind it isn't independently confirmed (e.g. by the targeted repro above catching the failure and then not catching it after the fix), don't ship it as-is — an unverified "shouldn't hurt" change is worse than no change, because it adds a real behavior diff without evidence it fixes anything, and makes the next diagnosis harder.
  - A CI run passing after a diagnostic-only change (timing logs, extra assertions — anything that can't itself alter pass/fail) is not confirmation of anything. A flake that fails a minority of runs is *expected* to pass most of the time with or without the change; treat a pass as "no data collected," not "resolved," and say so plainly rather than letting a green checkmark read as a fix. See ADR 0021.
...
- This repo has a `graphify-out/graph.json` knowledge graph. Before grepping or reading raw source to answer a codebase question, check the graph first (`graphify query "<question>"`, `graphify explain "<concept>"`, `graphify path "<A>" "<B>"`). If the graph doesn't surface enough context — thin coverage, a stale answer, or the question falls outside what got extracted — fall back to a wider grep/file search rather than treating the graph as the ceiling. When dispatching a subagent (Explore, general-purpose) for code exploration, put this same graphify-first-then-expand instruction in its prompt: subagents don't inherit this file automatically. The graph is not rebuilt automatically on every change — updating it (`graphify update .`) is a manual, explicit ask, not something to assume happened after recent edits.
